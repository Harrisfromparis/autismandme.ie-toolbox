...
async function handleDecisionFromQuery() {
  const decision = String(wixLocationFrontend.query.decision || "").trim();
  if (!decision || !runId || !DECISIONS.has(decision)) return;

  if (!run || !(run.status === "awaiting-approval" || run.status === "changes-requested")) {
    clearDecisionQuery();
    return;
  }

  const notes = decodeQueryText(wixLocationFrontend.query.notes || "").slice(0, 1500);
  setText("#approvalStatusText", decision === "approve" ? "Publishing…" : "Saving teacher decision…");
  setDecisionEnabled(false);

  const result = await decideGovernedPathway(runId, decision, {
    notes,
  ...
  setTimeout(() => clearDecisionQuery(), 250);
}

function clearDecisionQuery() {
  wixLocationFrontend.to(
    `/pathway-preview?pathwayId=${encodeURIComponent(pathwayId)}&runId=${encodeURIComponent(runId)}`
  );
}
...
function wireOptionalDecisionButtons() {
...
function wireButton(selector, decision) {
  try {
    $w(selector).onClick(async () => {
      ...
      const url = new URL("/pathway-preview", "https://www.autismandme.ie");
      ...
      wixLocationFrontend.to(`${url.pathname}?${url.searchParams.toString()}`);
    });
  } catch (error) {
    // The custom approval bar means editor buttons are optional.
  }
}
...
function setDecisionEnabled(enabled) {
  for (const selector of ["#approvePathwayButton", "#requestChangesButton", "#rejectPathwayButton"]) {
    try {
      if (enabled) $w(selector).enable(); else $w(selector).disable();
    } catch (error) {
      // custom bar handles decisions when buttons are absent
    }
  }
}
